...
> **注意**：在撰写本文时，Azure AI Studio 为预览版，目前正在积极开发中。 服务的某些元素可能与描述不完全一致，并且某些功能可能无法正常使用。
...
1. 在“生成”**** 页上选择“+ 新建项目”****。 然后，在“创建新项目”向导中，使用以下设置创建项目****：
    - 项目名称****：项目的唯一名称**
    - AI 中心****：使用以下设置创建新资源：**
        - AI 中心名称****：唯一的名称**
        - **Azure 订阅**：Azure 订阅**
...
1. 创建一个名为 `text-embedding-ada-002` 的 text-embedding-ada-002**** 模型的新部署（使用实时终结点）。 设置“高级”**** 选项以使用默认内容筛选器并将每分钟令牌数 (TPM) 限制为 5K****。
1. 创建一个名为 `gpt-35-turbo` 的 gpt-35-turbo**** 模型的新部署。 设置“高级”**** 选项以使用默认内容筛选器并将每分钟令牌数 (TPM) 限制为 5K****。

> **注意**：减少 TPM 有助于避免过度使用你正在使用的订阅中的可用配额。 对于本练习中使用的数据，5,000 TPM 已经足够。
...
1. 选择“+ 新建数据”****，然后通过上传 brochures**** 文件夹来添加新的数据源连接（选择上传文件夹** 而不是** 文件的选项）。 将新数据源命名为 brochures****。
