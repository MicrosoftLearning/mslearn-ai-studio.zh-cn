...
在本练习中，使用 Azure AI Foundry 门户将自定义数据集成到生成式 AI 提示流中。 你还将了解如何使用 Azure AI Foundry 和 Azure OpenAI SDK，在客户端应用中实现 RAG 模式。
...
1. 在左侧导航窗格中，选择“**操场**”页。
1. 在“聊天”页的“设置”窗格中，确保已选择“**GPT-4**”模型部署。 然后，在主聊天会话面板中提交提示“`Where can I stay in New York?`”
...
1. 通过克隆库中的“基于数据的多轮问答”**** 示例来创建新的提示流。 将此示例的克隆保存在名为 `brochure-flow` 的文件夹中。
    ...
    <details> 
        <summary><font color="red"><b>故障排除提示</b>：权限错误</font></summary>
        <p>如果新建提示流时收到权限错误，请尝试执行以下操作进行故障排除：</p>
        <ul>
            <li>在Azure 门户中的 Azure AI Foundry 中心资源组中，选择 AI 服务资源。</li>
            <li>在“资源管理”<b></b>下的“标识”<b></b>选项卡中，确保<b>系统分配</b>的托管标识状态为<b>开启</b>。</li>
            <li>在 Azure AI Foundry 中心的资源组中，选择存储账户</li>
            <li>在“访问控制” (IAM) <b></b>页面，添加角色分配，将<b>存储 Blob 数据读取者</b>角色分配给 Azure AI 服务资源的托管标识。</li>
            <li>等待角色分配完成后，重试上一步。</li>
        </ul>
    </details>

1. 当提示流设计器页面打开时，查看 brochure-flow****。 其图应与下图相似：
...
1. 使用“启动计算会话”按钮启动流的运行时计算。****
    ...
    等待计算会话启动完成。 这为提示流提供计算上下文。 在等待期间，请在“流”**** 选项卡中查看流中的工具部分。

    >**注意**：由于基础设施和容量限制，计算会话可能会在高需求时段启动失败。 如果发生这种情况，可以跳过使用提示流，直接开始任务 **：使用 Azure AI Foundry 和 Azure OpenAI SDK 创建 RAG 客户端应用**。
...
1. 查看流中每个工具的输出，注意流中的每个工具如何对其输入进行操作以准备上下文化提示并获得相应的响应。
    ...
    现在，你已拥有一个工作提示流，该流使用 Azure AI 搜索索引实现 RAG 模式。 若要了解如何部署和使用提示流的更多信息，请参阅 [Azure AI Foundry 文档](https://learn.microsoft.com/azure/ai-foundry/how-to/flow-deploy)。

## 使用 Azure AI Foundry 和 Azure OpenAI SDK 创建 RAG 客户端应用程序

虽然提示流是封装基于 RAG 的应用程序中的模型和数据的理想方式，但也可以使用 Azure AI Foundry 和 Azure OpenAI SDK 在客户端应用程序中实现 RAG 模式。 我们将在一个简单的示例中探索代码实现。
...
尽可能利用可用资源创建数据源，并将其整合到提示流或客户端应用程序中。 提交只能依据所选数据集回答的提示，测试解决方案！
